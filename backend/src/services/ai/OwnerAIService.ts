...
export class OwnerAIService {
  async chat(message: string, historyContext: string | any[], username: string, orgId: string = 'default'): Promise<string> {
    logger.info(`OwnerAIService: Processing chat request for user: ${username} in org: ${orgId}`);
...
Do not include any other text or markdown formatting.`;
      ...
      try {
        const classificationText = await aiService.generate(
...
- If the owner wrote in a specific language (like Odia, Hindi, or English), respond in the same language if possible.`;
      ...
      let finalResponse = '';
      try {
        finalResponse = await aiService.generate(
          formattingPrompt,
          `Format response for: ${message}`,
          aiProvider,
          classificationModel
        );
      ...
      logger.error(`OwnerAIService Error: ${err.message}`);
      return "An error occurred while compiling your request. Please try again.";
    }
  }
}
export default OwnerAIService;
