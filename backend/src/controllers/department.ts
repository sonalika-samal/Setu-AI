...
export class DepartmentController {
  async list(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const depts = await DepartmentModel.find({ orgId }).sort({ name: 1 });
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const existingName = await DepartmentModel.findOne({ orgId, name: { $regex: new RegExp(`^${name}$`, 'i') } });
      const existingCode = await DepartmentModel.findOne({ orgId, code: { $regex: new RegExp(`^${code}$`, 'i') } });
      ...
      await loggingService.logActivity(
        ...
        'Department Created',
        `Department ${name} (${code.toUpperCase()}) was created.`
      );
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (name) {
        const existingName = await DepartmentModel.findOne({ _id: { $ne: id }, name: { $regex: new RegExp(`^${name}$`, 'i') } });
        if (existingName) {
      ...
      if (code) {
        const existingCode = await DepartmentModel.findOne({ _id: { $ne: id }, code: { $regex: new RegExp(`^${code}$`, 'i') } });
        if (existingCode) {
      ...
      if (name) {
        await UserModel.updateMany({ department_id: id }, { department_name: name });
      }
      ...
      await loggingService.logActivity(
        user?.username || 'system',
        'Department Updated',
        `Department ${dept.name} details were updated.`
      );
  ...
  async delete(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const otherDept = await DepartmentModel.findOne({ code: 'OTHER' });
      if (!otherDept) {
      ...
      await loggingService.logActivity(
        user?.username || 'system',
        'Department Deleted',
        `Department ${dept.name} was deleted. Assigned workers were moved to "Other".`
      );
  ...
  async moveWorkers(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const targetDept = await DepartmentModel.findById(targetDepartmentId);
      if (!targetDept) {
      ...
      await loggingService.logActivity(
        user?.username || 'system',
        'Workers Moved',
        `Moved ${workerIds.length} workers to department ${targetDept.name}.`
      );
  ...
  async getSummaryStats(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const summaries = await Promise.all(depts.map(async (dept) => {
        const workers = await UserModel.find({ department_id: dept._id, role: 'Worker' }).lean();
  ...
  async getDepartmentDetails(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      ...
      const workers = await UserModel.find({ department_id: id, role: 'Worker' }).sort({ name: 1 }).lean();
      const workerIds = workers.map(w => w._id.toString());
      ...
      const totalCompleted = await TaskModel.countDocuments({
        worker_id: { $in: workerIds },
        task_status: 'Completed'
      });
      ...
      const totalAssigned = await TaskModel.countDocuments({
        worker_id: { $in: workerIds }
      });
