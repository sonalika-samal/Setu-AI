 ...
 */
function parseNaturalLanguageDate(dateStr) {
    const now = new Date();
    if (!dateStr) {
...
function cleanAndParseJSON(input) {
    ...
    try {
        ...
        logger_1.logger.error(`cleanAndParseJSON: Failed to parse JSON content from LLM: ${error.message}`);
        logger_1.logger.error(`Original output content was: ${input}`);
        return null;
    }
}
        ...
        try {
            ...
            if (messageType === 'text') {
                ...
                try {
                    ...
                    const tempFilePath = path_1.default.join(tempDir, `temp_voice_${Date.now()}_${crypto_1.default.randomBytes(4).toString('hex')}.ogg`);
                    ...
                    try {
                        ...
                        if (fs_1.default.existsSync(tempFilePath)) {
                            try {
                                ...
                                logger_1.logger.warn(`Failed to delete temporary audio file: ${delErr.message}`);
                            }
                        }
                    }
                }
                ...
                logger_1.logger.warn(`processWebhook: Message type ${messageType} is not supported. Ingestion cancelled.`);
            ...
            const incomingMsgLog = await MessageLog_1.MessageLogModel.create({
                message_id: messageId,
                sender: senderPhone,
            ...
            if (senderRole === 'Worker') {
                ...
                const cleanSenderPhone = senderPhone.replace('+', '').trim();
                const allTasks = await Task_1.TaskModel.find({
                    $or: [
                ...
                if (isProofUpload) {
                    ...
                    try {
                        ...
                        logger_1.logger.error(`Failed to download proof media from Meta: ${dlErr.message}`);
                        logDoc.processing_status = 'failed';
                        await logDoc.save();
                        this.emitSocketUpdate(io, logDoc);
                        return;
                    ...
                    if (matchedTask) {
                        ...
                        const savePath = path_1.default.join(uploadsDir, filename);
                        fs_1.default.writeFileSync(savePath, fileBuffer);
                        const localMediaUrl = `http://localhost:5000/uploads/${filename}`;
                        // Append proof to matched task
                        if (!matchedTask.proof_of_work)
                            matchedTask.proof_of_work = [];
...
Output exactly one of the words: Started, Completed, or More Details Asked. Do not output anything else.`;
                            ...
                            try {
                                ...
                                logger_1.logger.error(`Failed to classify caption status: ${err.message}`);
                            }
                            if (classifiedStatus) {
                                ...
                                if (classifiedStatus === 'Completed') {
                                    try {
                                        ReminderService_1.ReminderService.cancelTaskReminders(matchedTask._id.toString());
                                    }
                                    catch (reminderErr) {
                                        logger_1.logger.error(`Failed to cancel reminders on worker completion: ${reminderErr.message}`);
                                    }
                                }
                                // Timeline Entry
                        ...
                        await whatsAppService.sendMessage(senderPhone, `Successfully uploaded proof of work for Task ${matchedTask.taskId || matchedTask._id}. Admin/Owner will review.`);
                        logDoc.processing_status = 'completed';
                        logDoc.delivery_status = 'processed';
                        await logDoc.save();
                        this.emitSocketUpdate(io, logDoc);
                        ...
                        await workerDoc.save();
                        logger_1.logger.warn(`Proof Upload: Could not resolve target Task ID for worker ${workerDoc.name}. Saved pending proof.`);
                        await whatsAppService.sendMessage(senderPhone, "Thank you for sending the file. However, I could not determine which Task this proof is for. Please reply with the Task ID to link it.");
                        logDoc.processing_status = 'completed';
                        logDoc.delivery_status = 'processed';
                        await logDoc.save();
                        this.emitSocketUpdate(io, logDoc);
                ...
                if (isCheckIn) {
                    ...
                    try {
                        ...
                        logger_1.logger.error(`Failed to send check-in confirmation to ${senderPhone}: ${waErr.message}`);
                    }
                    logDoc.processing_status = 'completed';
                    logDoc.delivery_status = 'processed';
                    await logDoc.save();
                ...
                if (isCheckOut) {
                    ...
                    try {
                        ...
                        logger_1.logger.error(`Failed to send check-out confirmation to ${senderPhone}: ${waErr.message}`);
                    }
                    logDoc.processing_status = 'completed';
                    logDoc.delivery_status = 'processed';
                    await logDoc.save();
                ...
                // --- WORKER REPLY FLOW ---
                const recentMessages = await MessageLog_1.MessageLogModel.find({
                ...
                const pendingProof = workerDoc.pending_proof;
                if (pendingProof && pendingProof.media_url && matchedTaskId) {
                    const cleanSenderPhone = senderPhone.replace('+', '').trim();
                    const targetTask = await Task_1.TaskModel.findOne({
                    ...
                    if (targetTask) {
                        ...
                        try {
                            ...
                            if (oldFilename && fs_1.default.existsSync(path_1.default.join(uploadsDir, oldFilename))) {
                                const ext = oldFilename.split('.').pop() || 'bin';
                                const newFilename = `${targetTask.taskId || targetTask._id}_proof_${Date.now()}.${ext}`;
                                fs_1.default.renameSync(path_1.default.join(uploadsDir, oldFilename), path_1.default.join(uploadsDir, newFilename));
                                localMediaUrl = `http://localhost:5000/uploads/${newFilename}`;
                                filename = newFilename;
                            }
                        }
                        catch (renameErr) {
                            logger_1.logger.warn(`Failed to rename pending proof file: ${renameErr.message}`);
                        }
                        // Append proof to matched task
                        if (!targetTask.proof_of_work)
                            targetTask.proof_of_work = [];
                        ...
                        // Log Activity
                        await loggingService.logActivity(senderUsername, 'Proof Uploaded', `Worker ${workerDoc.name} linked proof of work file "${pendingProof.file_name}" to task ${targetTask.taskId || targetTask._id}.`);
                        // Emit Socket.IO event
                        if (io) {
                            io.emit('task:updated', { id: targetTask._id.toString() });
                            io.emit('notification:received', notification);
                        }
                        await whatsAppService.sendMessage(senderPhone, `Successfully linked proof of work to Task ${targetTask.taskId || targetTask._id}. Admin/Owner will review.`);
                ...
                if (isJustTaskId) {
                    ...
                    const lastIncomingMessages = await MessageLog_1.MessageLogModel.find({
                        sender: senderPhone,
                        direction: 'incoming',
                        type: 'text',
                    ...
                    if (intentMsg && intentMsg.message) {
                        ...
                        logger_1.logger.info(`WebhookService: Combined current Task ID message with previous message from last 5: "${textToClassify}"`);
                    }
                }
...
Output exactly one word: QUESTION or UPDATE. Do not include any other text.`;
                ...
                try {
                    ...
                    logger_1.logger.error(`Failed to classify message type: ${aiErr.message}`);
                }
...
${allTasks[0].taskId || '03082026T4'} Completed`;
                ...
                if (matchedTaskId) {
                    const cleanSenderPhone = senderPhone.replace('+', '').trim();
                    selectedTask = await Task_1.TaskModel.findOne({
                        taskId: matchedTaskId,
                        $or: [
                            { worker_phone: senderPhone },
                            { worker_phone: cleanSenderPhone },
...
Output exactly one of the words: Started, Completed, or More Details Asked. Do not output anything else.`;
                ...
                try {
                    ...
                    logger_1.logger.error(`Failed to send status update confirmation to worker: ${waErr.message}`);
                }
                // Cancel smart reminders if status is Completed
                if (classifiedStatus === 'Completed') {
                    try {
                        ReminderService_1.ReminderService.cancelTaskReminders(selectedTask._id.toString());
                    }
                    catch (reminderErr) {
                        logger_1.logger.error(`Failed to cancel reminders on worker completion: ${reminderErr.message}`);
                    }
                ...
                if (classifiedStatus === 'More Details Asked') {
                    try {
                        ...
                        logger_1.logger.error(`Failed to reply to worker: ${waErr.message}`);
                    }
                    ...
                    if (ownerPhoneNum) {
                        try {
                            ...
                            logger_1.logger.error(`Failed to notify owner: ${waErr.message}`);
                        }
                    }
                }
                logDoc.processing_status = 'completed';
...
[
  {
    ...
    "deadline": "Deadline"
                    ...
                    if (tasks.length === 0) {
                        logger_1.logger.warn('processWebhook: No tasks extracted. Alerting Owner.');
                        await whatsAppService.sendMessage(senderPhone, `Sorry, I could not confidently extract any tasks from your request. Could you please rephrase it?\n\nReceived: "${extractedText}"`);
                        logDoc.processing_status = 'failed';
                        await logDoc.save();
                        this.emitSocketUpdate(io, logDoc);
                        return;
                    ...
                    for (let i = 0; i < tasks.length; i++) {
                        ...
                        try {
                            ...
                            logger_1.logger.error(`Failed to assign task index ${i}: ${taskErr.message}`);
                    ...
                    logger_1.logger.info(`Owner confirmation sent to ${senderPhone}.`);
                    logDoc.processing_status = 'completed';
                    logDoc.delivery_status = 'processed';
                    await logDoc.save();
                    this.emitSocketUpdate(io, logDoc);
                    ...
                    await logDoc.save();
                    this.emitSocketUpdate(io, logDoc);
                    const recentOwnerMessages = await MessageLog_1.MessageLogModel.find({
                        $or: [{ sender: senderPhone }, { receiver: senderPhone }]
                    })
                        .sort({ timestamp: -1 })
                        .limit(15)
                    ...
                    const finalResponse = await ownerAIService.chat(extractedText, recentOwnerMessages.reverse().map(m => ({
            ...
            logger_1.logger.error(`processWebhook: Fatal pipeline execution failure: ${err.message}`);
            if (logDoc) {
                logDoc.processing_status = 'failed';
                await logDoc.save();
                this.emitSocketUpdate(io, logDoc);
            ...
            try {
                ...
                logger_1.logger.error(`Failed to create failure notification: ${notifErr.message}`);
            }
            // Create Activity Log for failure
            await loggingService.logActivity('system', 'Webhook Failure', `Fatal webhook error: ${err.message}`).catch(e => logger_1.logger.error(`Failed to log activity: ${e.message}`));
        }
    }
    /**
     * Helper to process a single task assignment safely in isolation
     */
    async processSingleTaskAssignment(task, ownerPhone, ownerUsername, creds, io) {
        ...
        const createdTask = await Task_1.TaskModel.create({
        ...
        try {
            ...
            if (isWorkerUnavailable && ownerPhone) {
                ...
                try {
                    ...
                    logger_1.logger.info(`Dispatched worker unavailability alert to Owner ${ownerPhone}`);
                }
                catch (ownerAlertErr) {
                    logger_1.logger.error(`Failed to dispatch availability alert to owner: ${ownerAlertErr.message}`);
                }
            }
            // Start smart reminder scheduling
            try {
                await ReminderService_1.ReminderService.scheduleTaskReminders(createdTask);
            }
            catch (remErr) {
                logger_1.logger.error(`Failed to schedule task reminders: ${remErr.message}`);
            }
        }
        catch (sendErr) {
