...
class SuperAdminController {
    ...
    async createOrg(req, res, next) {
        try {
            ...
            const existingOrg = await Organisation_1.OrganisationModel.findOne({ orgId });
            if (existingOrg) {
                res.status(400).json({ message: `Organisation ID "${orgId}" already exists.` });
                return;
            }
            // Create Organisation
            const newOrg = await Organisation_1.OrganisationModel.create({
                orgId,
                name,
            ...
            await Department_1.DepartmentModel.create({
                orgId,
                name: 'Other',
                code: 'OTHER',
            ...
            await Credential_1.CredentialModel.create({
                orgId,
                key: `${orgId}_config`,
                businessName: name,
                taskAssignmentTemplate: defaultTemplate
            });
            logger_1.logger.info(`SuperAdmin created organisation: ${name} (${orgId}) with admin: ${adminUsername}`);
            res.status(201).json({
    ...
    async listOrgs(req, res, next) {
        try {
            const orgs = await Organisation_1.OrganisationModel.find().sort({ createdAt: -1 }).lean();
            // Enrich with user & task counts
            const enrichedOrgs = await Promise.all(orgs.map(async (org) => {
                const userCount = await User_1.UserModel.countDocuments({ orgId: org.orgId });
                const taskCount = await Task_1.TaskModel.countDocuments({ orgId: org.orgId });
                return {
                    ...org,
                    userCount,
                    taskCount
     ...
     */
    async toggleOrgStatus(req, res, next) {
        try {
            ...
            const org = await Organisation_1.OrganisationModel.findOneAndUpdate({ orgId }, { $set: { isActive } }, { new: true });
            if (!org) {
                res.status(404).json({ message: 'Organisation not found.' });
                return;
            }
            logger_1.logger.info(`SuperAdmin updated org ${orgId} active status to: ${isActive}`);
    ...
    async updateOrgPlan(req, res, next) {
        try {
            ...
            const org = await Organisation_1.OrganisationModel.findOneAndUpdate({ orgId }, { $set: { plan } }, { new: true });
            if (!org) {
                res.status(404).json({ message: 'Organisation not found.' });
                return;
            }
            logger_1.logger.info(`SuperAdmin updated org ${orgId} plan to: ${plan}`);
