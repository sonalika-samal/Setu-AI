...
class TaskController {
    ...
    async updateTaskStatus(req, res, next) {
        try {
            ...
            const task = await taskService.updateTaskStatus(id, status, user?.username || 'system');
            if (!task) {
    ...
    async updateTask(req, res, next) {
        try {
            ...
            const user = req.user;
            const task = await taskService.updateTask(id, updateData, user?.username || 'system');
            if (!task) {
                res.status(404).json({ message: 'Task not found' });
                return;
            }
    ...
    async updateWorkerAvailability(req, res, next) {
        try {
            const { id } = req.params;
            ...
            if (!worker.availability_history)
                worker.availability_history = [];
    ...
    async manualUpdateTaskStatus(req, res, next) {
        try {
            const { id } = req.params;
    ...
    async getAttendanceStats(req, res, next) {
        try {
            ...
            try {
                ...
                req.app.get('logger')?.error(`Failed to run inactivity checker: ${checkerErr.message}`);
            }
            const { UserModel } = require('../models/User');
    ...
    async reviewProof(req, res, next) {
        try {
            ...
            const proof = task.proof_of_work.id(proofId);
            if (!proof) {
            ...
            if (!proof.approval_history)
                proof.approval_history = [];
            ...
            await loggingService.logActivity(user?.username || 'system', 'Proof Reviewed', `Proof of work ${proofId} on task ${task.taskId || task._id} was ${status.toLowerCase()} by ${user?.username || 'system'}.`);
            const io = req.app.get('io');
            if (io) {
    ...
    async reviewProofById(req, res, next) {
        try {
            ...
            const task = await Task_1.TaskModel.findOne({ "proof_of_work._id": proofId });
            if (!task) {
