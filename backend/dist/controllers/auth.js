...
class AuthController {
    async login(req, res, next) {
        try {
            ...
            if (!user) {
                const failedUser = await User_1.UserModel.findOne({ username });
                if (failedUser) {
            ...
            const dbUser = await User_1.UserModel.findById(user.id);
    ...
    async getAdminsOwners(req, res, next) {
        try {
    ...
    async addAdminOwner(req, res, next) {
        try {
            const { username, password, name, phone, role } = req.body;
    ...
    async deleteAdminOwner(req, res, next) {
        try {
            const { id } = req.params;
            const user = req.user;
    ...
    async refreshToken(req, res, next) {
        try {
            ...
            const storedToken = await RefreshToken_1.RefreshTokenModel.findOne({ token: refreshToken });
            if (!storedToken) {
                res.status(401).json({ message: 'Invalid refresh token.' });
                return;
            }
            if (new Date() > storedToken.expires_at) {
                await RefreshToken_1.RefreshTokenModel.deleteOne({ _id: storedToken._id });
                res.status(401).json({ message: 'Refresh token has expired.' });
                return;
            }
            const dbUser = await User_1.UserModel.findById(storedToken.user_id);
            if (!dbUser || dbUser.account_status === 'Disabled') {
    ...
    async resetPassword(req, res, next) {
        try {
            ...
            const targetUser = await User_1.UserModel.findById(userId);
            if (!targetUser) {
    ...
    async forceLogout(req, res, next) {
        try {
            const { id } = req.params;
            const executor = req.user;
            const targetUser = await User_1.UserModel.findById(id);
            if (!targetUser) {
                res.status(404).json({ message: 'User not found.' });
                return;
            }
            ...
            targetUser.token_version++;
            await targetUser.save();
            await RefreshToken_1.RefreshTokenModel.deleteMany({ user_id: id });
            await SecurityLog_1.SecurityLogModel.create({
                user_id: targetUser._id.toString(),
                username: targetUser.username,
                action: 'Force Logout',
    ...
    async getLoginHistory(req, res, next) {
        try {
            const history = await LoginHistory_1.LoginHistoryModel.find().sort({ timestamp: -1 }).limit(100);
    ...
    async getSecurityLogs(req, res, next) {
        try {
            const logs = await SecurityLog_1.SecurityLogModel.find().sort({ timestamp: -1 }).limit(100);
    ...
    async getActiveSessions(req, res, next) {
        try {
            const sessions = await RefreshToken_1.RefreshTokenModel.find({ expires_at: { $gt: new Date() } })
    ...
    async revokeSession(req, res, next) {
        try {
            const { id } = req.params;
            const session = await RefreshToken_1.RefreshTokenModel.findById(id);
            if (!session) {
            ...
            await RefreshToken_1.RefreshTokenModel.findByIdAndDelete(id);
            await SecurityLog_1.SecurityLogModel.create({
                user_id: targetUserId,
                username: targetUser ? targetUser.username : 'unknown',
                action: 'Session Revoked',
                details: `Active session revoked by ${req.user?.username || 'system'}.`
            });
    ...
    async bulkWorkerStatus(req, res, next) {
        try {
            ...
            await User_1.UserModel.updateMany({ _id: { $in: workerIds }, role: 'Worker' }, { worker_status: status });
